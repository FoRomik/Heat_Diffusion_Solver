...
/*
  ...
  dimension n must be of the form 2^j + 1 for some integer j. (j is
...
*/
  ...
  rstrct(irho[ngrid],u,nn,C);/* coarsens rhs (u at this point) to irho on mesh size nn */
  
  /***continue setting up coarser grids down to coarsest level***/
  while (nn > 3) { 
    nn=nn/2+1; 
    irho[--ngrid]=dmatrix(1,nn,1,nn);
    rstrct(irho[ngrid],irho[ngrid+1],nn,C); 
  }
  
  /***now setup and solve coarsest level iu[1],irhs[1] ***/
  nn=3;
  iu[1]=dmatrix(1,nn,1,nn);   /* iu[1] is the coarsest grid */
  irhs[1]=dmatrix(1,nn,1,nn);
  /* slvsml is the coarse grid solver */
  slvsml(iu[1],irho[1],C);      /* solve the small system directly */
  ...
  for (j=2;j<=ngrid;j++) {        /* loop over coarse to fine, starting at level 2 */
    ...
    */
    ...
    /* jcycle determines how many vcycles */
	...
	rstrct(irhs[jj-1],ires[jj],nf,C);  /* restrict residuals as rhs of next coarsest scale */
	fill0(iu[jj-1],nf);              /* set the initial solution guess to zero */
      } 
      slvsml(iu[1],irhs[1],C);                  /* solve the small problem exactly */
